...
void NewSegSiev(short *s,const unsigned long n,const long D,const long K, const long quot)
/* ensure: s[j] = (n-D+j is prime) for 0<=j<=2 D */
/* parameter kappa = 1/quot */
{
  mpz_class sqt, M2kap, sqtR, kmpz;
  ...
  for(long M = K*D+1; M <=x; M+=(2*R+1)) { //unabh: M, K, D, R
    ...
    cainv  = (ainv*c)%q;
		...
		#pragma omp parallel
		{
			...
			long chunksize = ceil((1.0*(k+2)) / omp_get_num_threads());
			DoSieving(tid, chunksize, k, q, m0, M, R, n, D, ainv, cainv, s);
		} // parallel part end
  }
}
