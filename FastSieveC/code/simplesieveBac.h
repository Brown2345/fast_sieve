short *smallprimeOrig=NULL;
unsigned long smallMOrig;

void SimpleSievOrig(short *p, unsigned long N)
...
void AllocSimpleSievOrig(unsigned long M)
{
  if(!smallprimeOrig || smallMOrig<M) {
    if(smallprimeOrig) free(smallprimeOrig);
    smallprimeOrig = (short *) calloc(M+1,sizeof(short));
    smallMOrig= M;
    SimpleSievOrig(smallprimeOrig,M);
  }
}

void SimpleSegSievOrig(short *s, unsigned long n, unsigned long D, unsigned long M)
...
void SimpleSegSievPOrig(short *s, unsigned long n, unsigned long D)
{
  unsigned long M;
  mpz_class sqt;
  mpz_class npD(n+D);
  ...
  SimpleSegSievOrig(s,n,D,M);
}

void SubSegSievOrig(short *s, unsigned long n, long D, long M)
/* assumes D+1 units of memory allocated at s */
/* ensures, for 0<=j<=D, that
  s[j] = 1   if n+j is coprime to all m<=M, 
  s[j] = 0   otherwise */
{
  ...
  AllocSimpleSievOrig(Dp);
  for(Mp=1; Mp<=M; Mp+=Dp+1) {
    SimpleSegSievPOrig(P,Mp,Dp);
...
void SegSievOrig(short *s, unsigned long n, unsigned long D)
{
  unsigned long M;
  mpz_class sqt;
  mpz_class npD(n+D);
