...
/* parameter kappa = 1/quot */
{
  ...
  fprintf(stderr,"Seconds calc End + Startval: %lf\n",seconds + useconds/1000000.0);
  ...
  fprintf(stderr,"end: %li chunksize: %li \n",end, chunksize);
}
...
int main(int argc, char *argv[])
{
  ...
  if (D >= n) D = n;
  unsigned long minBorder = pow(n,1.0/3)*pow(log(n),2.0/3);
  if (D <= minBorder) {
    fprintf(stderr, "set D to %lu", minBorder);
    D = minBorder;
  }
