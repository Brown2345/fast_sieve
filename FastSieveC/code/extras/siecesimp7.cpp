...
void DoSieving(short *s, long k, long q, long M, long m0, long Mp, long D, unsigned long n, long ainv, long cainv, int tid1, long chunksize) {

    long j,m;
		int tid = omp_get_thread_num();
    if (tid != tid1) fprintf(stderr,"ALARMARLARAMRALM");
		long r0 = -cainv;
		r0 -= tid * (k+2)/(chunksize*1.0) * ainv; //korrigieren
		r0 += (abs(r0)/q) * q;
    unsigned long np;

    long start = 0;
    long stop = k+2;// und hier
    for(r0= -cainv, j=0; j <= k+1; j++, r0 -= ainv) {
      if(r0 <= -q)
	r0 += q;
      for(m= m0+r0; m>=M; m-=q)
...
/*
    for(long j = 0; j < stop; j++, r0-=ainv) {
      ...
      for(long m=m0+r0+q; m<=Mp; m+=q)
		...
		r0 = -cainv + ainv;
		r0 += tid * (k+2)/(chunksize*1.0) * ainv;//hier grenze korrigieren
    r0 -= (abs(r0)/q) * q;
...
/* parameter kappa = 1/quot */
{
  ...
  long end = calcEnd(D, quot, n, K, npDtmp); //why leads recalc npD to float error?
  ...
  test.resize(end);
  for (int i = 0; i < end; i++) test.push_back(1);
  ...
  #pragma omp parallel num_threads(nthreads)
   {
    ...
    //for(M = K*D+1; M <=x; M+=(2*R+1)) { //unabh: M, K, D, R
      ...
      DoSieving(s, k, q, M, m0, Mp, D, n, ainv, cainv, tid, chunksize);
      M += 2*R+1;   
    }
  }
  int ok=0;
  for (int i = 0; i < end; i++) if (test[i] != 0) ok=1;
  if (ok != 0) {fprintf(stderr,"ALARM");return;}
